module Structure
  ...
  task :gene_mutations => :tsv do |organism|
    ...
    mutations.with_unnamed do
      mutations.through do |mutation, isoforms|
        changes[mutation] = isoforms.select{|mi| mi =~  /^#{protein}:[A-Z]\d+[A-Z*]/}.collect{|mi| mi.split(":").last}.first
      end
    end
    changes
  end
  ...
  task :gene_mutation_close_residues => :tsv do |distance, organism|
    ...
    pdbs.each do |pdb|
      ...
      pdb_close_contacts[pdb] = PdbTools.job(:pdb_close_positions, pdb, :pdb => pdb, :distance => distance).run
